import Alert from '@/packages/Alert/src/Alert.vue';
...
describe('Alert', () => {
	...
	it('O elemento deve esconder quando clicar em fechar', async () => {
		expect(wrapper.find('.el-alert').isVisible()).toBe(true);
		await wrapper.find('.el-alert__closebtn.el-icon-close').trigger('click');
		expect(wrapper.find('.el-alert').isVisible()).toBe(false);
	});
